...
| ショート動画 | 最大8カットを並べ、フェード / ワイプ / ディゾルブ / グリッチワイプ + Ken Burnsズームでつないだ動画を書き出し。9:16等の比率クロップ・長辺1280px・**MP4(H.264)優先**で、そのままリール/ショートに投稿可能。エフェクトと文字は動画全体にかかる |
| BGM・BPM同期 | 音声ファイルを読み込むと動画にAACトラックとして合成。BPM×拍数からカット間隔を自動設定（例: 128BPM×2拍=0.94s間隔） |
...
- [ ] カットごとのエフェクト切替・タイムライン編集
- [ ] GIF書き出し（X投稿用。ブラウザ内エンコーダの導入が必要）
