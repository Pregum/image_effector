...
Run workflow. On the first run, leave "apply schema.sql" on so the tables get created.
...
per cut). Connection techniques — `track-matte`, `radial-wipe`, `silhouette-match` and `match-cut` —
