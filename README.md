...
| レイヤー | ベース画像の上に画像・動画を何枚でも重ねられる。重ね順・不透明度・ブレンド・**素材の切り抜き**を各レイヤーごとに指定でき、動画は再生区間も切り出せる。合成した上からエフェクトが全体にかかる |
...
| `cw_open` / `cw_start` / `cw_hint` | 期間（`1d` `1w` `1m` / `demo`） | 時事クロスワードを開いた / 解き始めた / ヒントを使った |
