...
| AI生成（シーン） | llama-3.3-70b がテキストからシーン仕様(JSON)を設計し、ブラウザがCanvasでベクター風イラストとして描画。サンプル画像と同じ「味」をテキストから量産できる |
| 入出力 | ドラッグ&ドロップ / ファイル選択 / クリップボード貼り付け読み込み、PNG保存 |
| サンプル | プロシージャル自作のサンプル画像4種（夕日グリッド / ネオン路地 / 月と海 / 夕暮れ山並み）。「◧ サンプル」で切替、`/#sample=N`で直リンク可 |
