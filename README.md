...
| シェア | Xシェアボタン + OGP画像・Twitter Card + **レシピURL**（全設定を`#r=`にシリアライズ。画像は含まないので相手は自分の写真に同じルックを適用できる） |
...
| ギャラリー | 作品（元画像+エフェクトレシピ）をR2+D1に保存し、いつでも読み込んで再編集。**画像は既定で非公開**（一覧取得時に受け取る期限付き署名付きURLでのみ閲覧可） |
| 共有リンク | 作品ごとに `/w/<id>` の共有ページを発行。OGP付きで実際の作品がプレビュー展開され、開くとその画像+レシピでエディタが始まる。**約1日で自動失効**し、以降は410/401になる |
...
| 保護 | AI生成5回/分・ギャラリー保存10回/分・埋め込み解析20回/分のIPごとレート制限 (Durable Objects)。加えて**日次AI予算ガード**（8,000ニューロン/日＝無料枠10,000の80%で自分から停止）により、プランに関わらず従量課金の領域に入らない。`GET /api/budget` で当日の使用量を確認可 |
